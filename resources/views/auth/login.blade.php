...
@section('contenido')
    <div class="md:flex md:justify-center md:gap-10 md:items-center">
        <div class="md:w-6/12 p-5">
            <img class="rounded-lg " src="{{ asset('img/login.jpg')}}" alt="Imagen Login">
        </div>
        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
            <form method="POST" action="{{ route('login') }}">
                @csrf
                @if(session('mensaje'))
                    <p class="bg-red-500 font-bold text-white my-2 rounded-lg text-sm p-2 text-center">{{ session('mensaje')}}</p>
                @endif
                <div class="mb-5">
                ...
                <div class="mb-5 flex items-center">
                    <input type="checkbox" name="remember"><label class="text-gray-500 text-sm ml-2">Mantener Sesión Abierta</label>
                </div>
                <input 
                    type="submit"
                    value="Crear Cuenta"
